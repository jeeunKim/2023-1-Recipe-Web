...
<header class="head">
	...
	<section class = "up">
		...
		<c:if test = "${nickname != null}">
			...
			<%= (String) session.getAttribute("nickname")%>&nbsp님이 올린 레시피</span>
		</c:if>
	...
	<script>
		var fixsuccess = '<%=(Integer)session.getAttribute("fixsuccess")%>';
		var recipe_id2 = '<%=(Integer)session.getAttribute("recipe_id2")%>';
		  if(recipe_id2 != null){ 
			if(fixsuccess==1){ 
			   	 alert("레시피가 수정되었습니다.");
			   	 <% session.removeAttribute("fixsuccess");%>
				 window.location.href= "ViewMine.do?recipe_id="+ recipe_id2;
		    }
		  }
	</script>	
	...
	<c:forEach var="cnt" begin="1" end="${count}" step="1" varStatus="status">
		...
		<div class="card" >
		  <a href ="ViewMine.do?recipe_id=<%= recipe_id[many]%>"><img src=<%= img[many]%> class="card-img-top" alt="..."></a>
		  <div class="card-body">
		    <h5 class="card-title"><a href ="ViewMine.do?recipe_id=<%= recipe_id[many]%>" 
		    style= "text-decoration:none; color:black;"><%= recipe_name[many]%></a> 
			<a href ="MyRecipe.do?recipe_id=<%= recipe_id[many] %>&delete=<%= 1 %>" onClick="alert('정말로 삭제하시겠습니까?\n삭제하면 다시 복구할 수 없습니다.')" class="delete">삭제</a>
		    <a href ="fixView.do?recipe_id=<%= recipe_id[many] %>" class="fix">수정</a></h5>
